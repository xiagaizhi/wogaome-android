...
public class BindPhoneVu extends BaseVu<BindPhoneContract.Presenter> implements BindPhoneContract.IView {
        ...
        switch (v.getId()) {
            case R.id.bt_reset:
                if (CheckUtil.checkInputState(et_phone, null, et_verification_code, true)) {
                    mPersenter.bindPhone(et_phone.getText().toString(), et_verification_code.getText().toString());
                }
                break;
    ...
    @Override
    public void connectData(boolean isSafetyCheck, boolean isNeedReturnState, String phoneNumber) {
        this.isSafetyCheck = isSafetyCheck;
        if (isSafetyCheck) {
            titleName.setText("安全校验");
            et_phone.setFocusable(false);
            iv_clear_phone.setVisibility(View.GONE);
            if (!TextUtils.isEmpty(phoneNumber)) {
                et_phone.setText(phoneNumber.substring(0, 3) + "****" + phoneNumber.substring(7, phoneNumber.length()));
            }
        } else {
